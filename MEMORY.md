...
- 2026-06-13: Pause/resume is handled by the Space key in `src/js/input.js` and `PacmanGame.togglePause()` in `src/js/game.js`; paused updates must not advance game time, and `npm test` covers state preservation in `tests/pause.test.js`.
