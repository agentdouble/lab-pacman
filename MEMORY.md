...
- 2026-06-20: Maze variants are defined in `MAZE_DEFINITIONS` in `src/js/constants.js`; keep map-selection behavior wired through `Maze`/`PacmanGame`, and use `npm test` to verify layout dimensions, reachable pellets/bonuses/spawns, actor movement, and clearable win conditions.
- 2026-06-20: Character selection lives in `src/js/characters.js` and `src/js/character-selector.js`; selected profiles affect Pac-Man accent/identity, starting lives, speed, score multiplier, and power pellet duration, while the separate Pac-Man color selector controls the body color. Changing the selected character starts a fresh run.
